...
export const NewProjectPage = () => {
    ...
    // Models available to the tested key: null until fetched
    ...
    // Changing provider or key invalidates any previous test result + model list
    useEffect(() => {
    ...
    const handleTestLLMKey = async () => {
        ...
        try {
            ...
            if (data?.valid) {
                ...
                // Key works — fetch the models this key can actually use so the
                // user can pick one (default preselected).
                try {
                    const { data: m } = await apiService.getLLMModels(commercialProvider, keyToTest || null);
                    if (m?.valid && Array.isArray(m.models) && m.models.length) {
                        setAvailableModels(m.models);
                        setDefaultModel(m.default_model || m.models[0]);
                        setSelectedModel(m.default_model || m.models[0]);
                    }
                } catch { /* model list is an enhancement — key test result stands */ }
            } else {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const projectData = {
                ...
                // The user-selected model within the provider's series (not secret)
                llmModel: selectedLLM === 'commercial-api' ? (selectedModel || null) : null,
                selectedVM,
                codeLocation,
                                            ...
                                            <div className="mt-3">
                                                <label className="block text-sm font-medium text-gray-700 mb-1.5">
                                                    ...
                                                    <span className="ml-2 text-xs font-normal text-gray-400">
                                                        {availableModels.length} available to this key
                                                    </span>
                                                </label>
                                                <select
                                                    value={selectedModel}
