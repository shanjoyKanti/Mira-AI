...
const StatCard = ({ icon: Icon, value, label, color, iconBg, subLabel }) => (
  <div className={`group rounded-2xl p-6 border bg-white hover:shadow-lg hover:-translate-y-0.5 hover:border-primary-200 transition-all duration-200 ${color}`}>
    <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-4 shadow-sm group-hover:scale-105 transition-transform duration-200 ${iconBg}`}>
      <Icon className="w-5 h-5 text-white" aria-hidden="true" />
    </div>
    <p className="text-3xl font-extrabold mb-1 tabular-nums tracking-tight">{value}</p>
  ...
  useEffect(() => {
    ...
    const load = () => {
      Promise.all([
        ...
        setMigrationCounts(countMigrationStates(sorted));
        setTransforms(sorted.slice(0, 15));
  ...
  const handleCancelTransform = async (t) => {
    ...
    try {
      ...
      apiService.listTransforms().then(res => {
        ...
        setTransforms(tData.sort((a, b) => new Date(b.created_at) - new Date(a.created_at)).slice(0, 5));
    ...
    <DashboardLayout>
      <div className="space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 flex items-center gap-2.5 flex-wrap">
              <span>
                Welcome back{user?.username ? <>, <span className="bg-gradient-to-r from-primary-600 to-violet-600 bg-clip-text text-transparent">{user.username}</span></> : ''}
              </span>
        ...
        {statsLoading ? (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[1,2,3,4].map(i => (
              <div key={i} className="rounded-2xl border h-32 skeleton-shimmer" />
            ))}
          </div>
        ) : isError ? (
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard icon={FolderOpen}   value={total}     label="Total Projects"  color="border-gray-200 text-gray-900"  iconBg="bg-gradient-to-br from-primary-500 to-violet-600" />
            <StatCard icon={CheckCircle}  value={migrationCounts ? completed : '–'} label="Completed"   subLabel="latest migration per project" color="border-gray-200 text-gray-900"  iconBg="bg-gradient-to-br from-green-500 to-emerald-600" />
            <StatCard icon={Activity}     value={migrationCounts ? inProg : '–'}    label="In Progress" subLabel="latest migration per project" color="border-gray-200 text-gray-900"  iconBg="bg-gradient-to-br from-violet-500 to-purple-600" />
            <StatCard icon={AlertCircle}  value={migrationCounts ? failed : '–'}    label="Failed"      subLabel="latest migration per project" color="border-gray-200 text-gray-900"  iconBg="bg-gradient-to-br from-red-500 to-rose-600" />
          </div>
        )}
                ...
                {transforms.map(t => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Cancel button - only shown for in-flight migrations.
                      ...
                      {status === 'running' && (
                        <button
                          ...
                          title="Cancel this running migration"
