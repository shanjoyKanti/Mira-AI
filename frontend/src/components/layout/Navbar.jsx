...
export const Navbar = ({ onMenuClick }) => {
  ...
  // Authenticated app chrome: a slim top bar that starts after the dark rail
  // (the rail carries the brand). Public pages keep the full-width branded bar.
  const shellClasses = isAuthenticated
    ? 'bg-white/80 backdrop-blur-xl border-b border-gray-200/80 fixed top-0 right-0 left-0 lg:left-60 z-30'
    : 'bg-white/80 backdrop-blur-xl border-b border-gray-200/80 fixed w-full z-50 top-0';

  return (
    <nav className={shellClasses} role="navigation" aria-label="Main navigation">
      <div className={isAuthenticated ? 'px-4 sm:px-6 lg:px-8' : 'max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'}>
        <div className={`flex justify-between ${isAuthenticated ? 'h-14' : 'h-16'}`}>
          <div className="flex items-center">
            ...
            {/* Brand lives in the rail for the app shell; keep it here for
                public pages and for mobile (where the rail is hidden). */}
            <span className={isAuthenticated ? 'lg:hidden' : ''}>
              <BrandLogo textClass={isAuthenticated ? 'text-lg' : 'text-xl sm:text-2xl'} />
            </span>
          </div>
